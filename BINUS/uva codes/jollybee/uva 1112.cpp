...
int dijkstra(int b, int exit, int timeLimit , int from) {
	...
	dist[from] = 0;
	int result =0;
	pq.push(make_pair(-dist[from], make_pair(from,from)));
	while(!pq.empty()) {
 
		pair<int,pair<int,int>> currentNode = pq.top();
		pq.pop();
		int currentFrom = currentNode.second.first;
		int currentTo = currentNode.second.second;
		int currentCost = -currentNode.first;
		if (visited[currentFrom][currentTo]) continue;
	
		visited[currentFrom][currentTo] = 1;
 			...
 			for(int i=0;i<PETA[currentTo].tujuan.size();i++){
 				if(currentCost + PETA[currentTo].waktu.at(i) < dist[PETA[currentTo].tujuan.at(i)]){
					...
					pq.push(make_pair(-dist[PETA[currentTo].tujuan.at(i)], make_pair(currentFrom,PETA[currentTo].tujuan.at(i)))); 					
 				}
 			}
 
 
			/*for(int k=0; k<4; k++) {
				int newR = currentR + dr[k];
				int newC = currentC + dc[k];
				if (newR < 1 || newR > R) continue;
				if (newC < 1 || newC > C) continue;
				if (visited[newR][newC]) continue;
				if (currentCost + PETA[newR][newC] < dist[newR][newC]) {
					// relax
					dist[newR][newC] = currentCost + PETA[newR][newC];
